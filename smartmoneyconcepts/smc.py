...
class smc:
    ...
    @classmethod
    def old_swing_highs_lows(cls, ohlc: DataFrame, swing_length: int = 50) -> DataFrame:
        """
    ...
    def bos_choch(
        ...
        bos, choch, level, broken = compute_bos_choch(swing_highs_lows.HighLow.to_numpy(),swing_highs_lows.Level.to_numpy(), ohlc_close, ohlc_high, ohlc_low, close_break)
...
def remove_consecutive_swings(highs, lows, swing_highs_lows):
    ...
    while True:
        ...
        index_to_remove = np.zeros(len(positions), dtype=np.bool_)
